...
import { getCategory, getSearchQuery, getUsersResults } from "@/redux-store/slices";
...
export default function UserCollection() {
    ...
    useEffect(() => {
        ...
        dispatch(fetchUsers({q: searchQuery, currentPage}));
    ...
    const nextHandler = () => {
        if(!UsersResults?.response) return;
        if(currentPage === getPages(UsersResults?.response?.total_count)) return;
        setCurrentPage(currentPage + 1);
    }

    const renderPagination = () => {
        const totalPages = getPages(UsersResults?.response?.total_count || 0);
