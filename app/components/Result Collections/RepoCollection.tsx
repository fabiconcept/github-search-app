...
import { getCategory, getRepositoriesResults, getSearchQuery } from "@/redux-store/slices";
...
export default function RepoCollection() {
    ...
    useEffect(() => {
        ...
        dispatch(fetchRepositories({q: searchQuery, currentPage}));
    ...
    useEffect(() => {
        ...
        setDisplayRepo(RepositoriesResults.response?.items);
    }, [RepositoriesResults.response]);
    ...
    const nextHandler = () => {
        if(!RepositoriesResults?.response) return;
        if(currentPage === getPages(RepositoriesResults?.response?.total_count)) return;
        setCurrentPage(currentPage + 1);
    }

    const renderPagination = () => {
        const totalPages = getPages(RepositoriesResults?.response?.total_count || 0);
    ...
    return (
        <div className="w-full relative">
            {!!(displayRepo.length > 0) && <div className="mb-4 text-xl opacity-70">Repositories results</div>}
            {RepositoriesResults.loading === "pending" && !!(displayRepo.length > 0) &&  <div className="absolute top-0 left-0 h-full w-full z-10 bg-white/5 backdrop-blur-md grid place-items-center">
                <div role="status">
            ...
            {!(displayRepo.length > 0) && Category.value ==="REPOSITORIES" && <div className="grid place-items-center p-8 gap-4 opacity-70 h-[20rem]object">
                <Image
            ...
            {!!(displayRepo.length > 0) && RepositoriesResults.response && RepositoriesResults.response.total_count > 15 && <div className="flex items-center justify-center gap-1 mt-6 text-xs">
                ...
                {renderPagination()}
                <div className={`flex gap-1 items-center px-4 cursor-default ${currentPage === getPages(RepositoriesResults.response.total_count) ? "opacity-40" : "text-green-500 cursor-pointer active:scale-90 select-none"}`} onClick={nextHandler}>
